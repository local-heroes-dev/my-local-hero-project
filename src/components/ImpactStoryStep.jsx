...
const ImpactStoryStep = ({ formData, updateFormData, nextStep, prevStep }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-3xl mx-auto bg-white p-6 rounded shadow mt-10 space-y-6">
      ...
      <div>
        <label className="block font-medium text-gray-700">Full Impact Story *</label>
        <textarea
          {...register("fullStory", { required: "Full story is required", minLength: { value: 100, message: "At least 100 characters" } })}
          className="w-full p-2 border border-gray-300 rounded"
          rows={6}
          placeholder="Tell us the full story of their impact..."
        />
        {errors.fullStory && <p className="text-red-500 text-sm">{errors.fullStory.message}</p>}
        <p className="text-xs text-gray-500 mt-1">{formData.fullStory.length} characters (min 100)</p>
      </div>

      <div>
        <label className="block font-medium text-gray-700">Impact Tags (Optional)</label>
        <input
          {...register("tags")}
          className="w-full p-2 border border-gray-300 rounded"
